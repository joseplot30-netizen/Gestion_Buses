...
<body class="bg-slate-950 text-slate-200 p-6 font-sans">
    ...
    <script>
        ...
        document.getElementById('formBus').onsubmit = async (e) => {
            e.preventDefault();
            const codigo = document.getElementById('b_codigo').value.trim();
            const placa = document.getElementById('b_placa').value.trim();
            const capacidad = document.getElementById('b_capacidad').value.trim();

            if (!codigo || !placa) {
                alert('Completa código y placa antes de enviar.');
                return;
            }
            ...
            let errMsg = 'Código de error ' + res.status;
            try {
            ...
            alert('Error al registrar unidad: ' + errMsg);
        };
        ...
        function abrirEditarUsuario(id, rol) {
            ...
            // Cargar datos del usuario/admin desde el backend
            const endpoint = rol === 'ADMIN' ? 'http://127.0.0.1:8000/admins' : 'http://127.0.0.1:8000/usuarios';
            fetch(endpoint)
                .then(res => res.json())
                .then(data => {
                    ...
                    if (user) {
                        ...
                        document.getElementById('edit_tipo').value = rol === 'ADMIN' ? 'admin' : 'user';
                        ...
                        // Mostrar/ocultar selector de bus según el rol
                        const busContainer = document.getElementById('bus_select_container');
                        if (rol === 'ADMIN') {
                            busContainer.style.display = 'none';
                            document.getElementById('edit_bus_id').value = '';
                        } else {
        ...
        function borrarUsuario(id, rol) {
            ...
            const mensaje = '¿Estás seguro de que quieres borrar este ' + tipo + '?';
            if (confirm(mensaje)) {
                const endpoint = rol === 'ADMIN' ? 'http://127.0.0.1:8000/admin/' + id : 'http://127.0.0.1:8000/usuario/' + id;
                fetch(endpoint, { method: 'DELETE' })
                    .then(res => {
                        if (res.ok) {
                            alert(tipo.charAt(0).toUpperCase() + tipo.slice(1) + ' borrado');
                            location.reload();
                        } else {
                            alert('Error borrando ' + tipo);
                        }
                    })
                    .catch(err => {
        ...
        document.getElementById('formEditarUsuario').onsubmit = async (e) => {
            ...
            const endpoint = tipo === 'admin' ? 'http://127.0.0.1:8000/admin/' + userId : 'http://127.0.0.1:8000/usuario/' + userId;

            try {
                const res = await fetch(endpoint, { method: 'PUT', body: fd });
                if (res.ok) {
                    alert(tipo === 'admin' ? 'Administrador actualizado' : 'Usuario actualizado');
                    location.reload();
                } else {
